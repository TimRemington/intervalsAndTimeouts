document.addEventListener('DOMContentLoaded', () => {
	document.getElementById("transition").addEventListener('click', change)
})

// 1. Add an event listener to the button on click
// 2. If either select box does not have an option selected, surface an error
// 3. When you click the button, make a change to the changing element
//   (a) If 'Color' is selected, change the color of the text
//   (b) If 'Background Color' is selected, change the background-color
//   (c) If 'Font Style' is selected, change the font-style
//   (d) If 'Font Weight' is selected, change the font-weight
// 4. Run the transitions in order. Make sure to nest the timeouts!
// 5. When you click the "Go!" button again, it should clear out the styles first before transition to new styles.

// Targeted Variables
let button = document.getElementById("transition")
let tranA = document.getElementById("change-A")
let tranB = document.getElementById("change-B")
let changing = document.getElementById("changing")
let error = document.getElementById("error")

let finalError = document.createTextNode("Error, please make the correct selection.")
error.appendChild(finalError)

// FUNCTION
function change (event) {

changing.removeAttribute("style")

if (tranA.value === "" || tranB.value === "") {
	error.hidden = false
} else {
	error.hidden = true
}

setTimeout (function() {
	if (tranA.value !== "") {
		modify(tranA.value)
	}
	setTimeout (function () {
		if (tranB.value !== "") {
			modify(tranB.value)
		}
	}, 1000)
}, 1000)

}

// FUNCTION
function modify (input) {
	switch (input) {
		case "color":
			changing.style.color = "red"
			break;
		case "background":
		changing.style.backgroundColor = "yellow"
			break;
		case "font-style":
			changing.style.fontStyle = "italic"
			break;
		case "font-weight":
			changing.style.fontWeight = "bold"
			break;
	}

}
